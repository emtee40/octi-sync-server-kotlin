...
- **Lock-order inversion in commit** — the §"Concurrency And Locking" plan order is `lifecycle → session → module → quota`, but `ModuleLifecycleService.commitModule` actually takes the device (module) lock first and then enters the session lock inside `consumeAndMoveCompletedBlob`. No deadlock today because no other path takes the reverse order — `PATCH`/`finalize` never re-enter the module lock. If the deferred per-module lifecycle flag is added and lets readers run concurrently with commits, this inversion needs to be resolved by hoisting session resolution out of the module lock.
